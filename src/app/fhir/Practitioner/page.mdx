...
<Row>
  <Col>
    ...
    Note that this endpoint **always creates** — it does not deduplicate on identifier, and `If-None-Exist` is not honored for Practitioners. For imports that may run more than once, use the [conditional update](#conditionally-update-a-practitioner) (`PUT /Practitioner?identifier=...`) instead, which creates the Practitioner when no match exists.
  </Col>
  <Col sticky>
