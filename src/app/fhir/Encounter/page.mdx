...
<Row>
  <Col>
    ...
    When no Encounter is found that matches the search criteria, a **new Encounter is created**. Currently only identifiers with system 'http://tiro.health/Encounter' are allowed for matching.
  </Col>
  <Col sticky>
    <CodeGroup title="Request" tag="PUT" label="/Encounter?identifier=http://tiro.health/Encounter|12345">
    ```bash {{ title: 'cURL' }}
    curl -X PUT "https://reports.tiro.health/fhir/r5/Encounter?identifier=http://tiro.health/Encounter|12345" \
      -H "Authorization: Basic {{apikey}}" \
      -H "Content-Type: application/fhir+json" \
      -d '{
        "resourceType": "Encounter",
    ...
    client.Update(encounter, new SearchParams().Where("identifier=http://tiro.health/Encounter|12345"));
    ```
    </CodeGroup>
    ```json {{ title: 'Response' }}
    {
      ...
      "identifier": [{
        "system": "http://tiro.health/Encounter",
        "value": "12345"
      }]
    }
    ```
