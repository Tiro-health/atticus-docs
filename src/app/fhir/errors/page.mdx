...
<Row>
  <Col>
    ...
    <Properties>
      ...
      <Property name="issue[].details" type="CodeableConcept" href="https://fhir.tiro.health/CodeSystem-operation-outcome-issue-detail.html">
        Carries the human-readable message in `details.text`, and — for some
        conditions — a stable coded detail in `details.coding` from the
        [`operation-outcome-issue-detail`](https://fhir.tiro.health/CodeSystem-operation-outcome-issue-detail.html)
        code system. Branch on `issue.code` and, when present, that coding;
        treat `details.text` as a display/log string only.
      </Property>
...
3. **`issue.details.coding`** — when a coding from the Tiro detail [`operation-outcome-issue-detail`](https://fhir.tiro.health/CodeSystem-operation-outcome-issue-detail.html) code system (`http://fhir.tiro.health/CodeSystem/operation-outcome-issue-detail`) is present, branch on its `code` for a precise condition. Treat it as a refinement: many errors expose only `issue.code` + `details.text`, so never depend on a coding being present.
