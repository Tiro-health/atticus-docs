...
The API key belongs to a [data tenant](/guides/data-tenants), which determines what data the request can reach.
  ...
  <Col sticky>
    ...
    curl -X POST https://auth.tiro.health/sessions \
      ...
      -d '{
        "scope": "patient/Patient.read patient/QuestionnaireResponse.*",
        "user": 456,
        "patient": 123
      }'
    ```
...
An API key authenticates your *system*, not a person: every request made with it acts as the service account tied to the key.

When work must be attributed to a specific end-user — the practitioner filling in a report — do not call the FHIR API with the key directly. Instead, create a **session** for that user via the [Session Management API](/api/session-management): the session user is identified in the request body (the `user` field, or a `fhirContext` entry of type `Practitioner` matched by FHIR identifier), and the browser session then carries the user's identity and scopes.

The identifier must match one on a [Practitioner](/fhir/Practitioner#create-a-new-practitioner) in your tenant, or the session can create the user on the fly with the `https://tiro.health/fhir/role/new` context role — see [FHIR Context](/api/session-management#fhir-context).
