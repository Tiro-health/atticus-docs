...
<Note>
  If you are currently calling the singular `/session` endpoints, see [Migrating from /session](#migrating-from-session). Those endpoints are deprecated.
</Note>
...
- **`Patient`, `Encounter`, `Practitioner`, and `DocumentReference`** are capped to **read-only by ID**: no create, update, delete, or search — regardless of what was requested. These resources are opened from the launch context and are materialised via [`fhirContext` `role/new`](#fhir-context), not via FHIR writes.
...
The same restriction applies to your API key's own FHIR calls in an EHR-managed tenant: `POST /fhir/r5/Patient` returns `403` — patients enter the tenant through session context resolution instead.
...
The singular `/session` endpoints are **deprecated**. They still work, but new integrations should use `/sessions`, and existing ones should move over. Both live on the same host, `auth.tiro.health` — only the paths and semantics change. (Older versions of these docs listed `reports.tiro.health` as the session host; that was never correct, it returns `404`.)
...
**The end-user moves into the request body.** `POST /session` identified the user with the `X-User-Id` header; `POST /sessions` takes a `user` field or a `fhirContext` entry of type `Practitioner` instead, and rejects requests without one (`422`).

**The handover token is returned in the response body.** `POST /sessions` responds `201` with `handover_token` in the JSON, and the cookie is only set at handover. You no longer read a cookie off the create response.
