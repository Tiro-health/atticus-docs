...
2. **Browser Handover**: The end-user's browser is sent to `/sessions/$handover` with that token and a `next` [Launch URL](#session-handover). The session is activated, a secure cookie is set in the browser, and the user lands on the task
...
sequenceDiagram
    ...
    Browser->>-Backend: GET post_submit_redirect<br/>?response=QuestionnaireResponse/456&task=Task/123
...
<Properties>
  <Property name="scope" type="string">
    Space-separated list of requested scopes (e.g., `"patient/Patient.read patient/QuestionnaireResponse.*"`). These scopes control what data the user can access during the session. A list of strings is also accepted. See [Scope-based Access Control](#scope-based-access-control).
  </Property>
...
<Properties>
  ...
  <Property name="fhir_server" type="object">
    The effective `scope` list granted to the session, after [downscoping](#scope-based-access-control). The `address` field is `null` for API-created sessions.
  </Property>
...
<Note>
  Submitting or cancelling sends the user back to `post_submit_redirect`; saving a draft or closing the tab does not. Don't treat the redirect as your only end-of-session signal — delete the session from your backend when you are done with it.
</Note>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL (external identifiers)' }}
    # Identify the patient and clinician by your own identifiers instead of
    # Tiro.health IDs — role/new creates them when they do not exist yet.
    curl -X POST https://auth.tiro.health/sessions \
      -u "<client_id>:<client_secret>" \
      -H "Content-Type: application/json" \
      -d '{
...
<Properties>
  ...
  <Property name="next" type="string">
    Where to send the user after activation. Use the [Launch URL](#session-handover) below to open a [Task](/fhir/Task).
  </Property>
</Properties>
...
Point `next` at `https://app.tiro.health/external/v1` — the entry point for opening the Tiro.health application from an external system. Note the host: this is a page on the **application** (`app.tiro.health`), not an endpoint on the auth server. It resolves the [Task](/fhir/Task) and sends the user to the right page for its current status. Use it rather than linking to an application page directly: the internal page URLs are an implementation detail and may change without notice.
...
<Properties>
  ...
  <Property name="viewMode" type="string">
    Optional. `embedded` renders the report without the surrounding application chrome (navigation, top bar) — use this inside iframes and [embedded browsers](/api/embedded-browsers). Defaults to the session's `deployment_mode`.
  </Property>
...
The launch page itself can fail: `400` when neither `task` nor `task:identifier` is provided (or the value is malformed), `404` when no task matches the identifier, `409` when more than one matches, and `422` when the task cannot be opened (status `failed`, or a non-`draft` task without a questionnaire response yet).
  ...
  <Col sticky>
    ...
    <CodeGroup title="Launch URLs for next">
...
Filters are currently honored for `Questionnaire` scopes only, where they narrow which report templates the session can access — see [Template Access and Filtering](#scope-based-access-control) below.
...
[SMART v2 permission syntax](https://hl7.org/fhir/smart-app-launch/scopes-and-launch-context.html) is also accepted: any combination of `c` (create), `r` (read), `u` (update), `d` (delete), `s` (search), e.g. `patient/QuestionnaireResponse.cruds`. The v1 forms are converted automatically (`.read` ⇒ `.rs`, `.write` ⇒ `.cud`, `.*` ⇒ `.cruds`), and the session's stored scopes (`fhir_server.scope`) are always in v2 form.
...
<Note>
  A Practitioner created via `role/new` starts with no project memberships — but they don't need any to pick a template: sessions carry your API key's template access on the `Questionnaire` scope (see [Template Access and Filtering](#scope-based-access-control)). Just make sure the session's scopes include `Questionnaire.read`.
</Note>
