...
<Note>
In **EHR-managed data tenants**, direct writes to `Patient`, `Encounter`, `Practitioner`, and `DocumentReference` are blocked (`403 Forbidden`). These resources are created automatically when you create a session with `fhirContext` items using the role `https://tiro.health/fhir/role/new` — see [FHIR Context](/api/session-management#fhir-context). The `Task` import works in every tenant.
</Note>
