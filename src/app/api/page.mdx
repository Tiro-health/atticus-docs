...
<Note>
In **EHR-managed data tenants**, *end-user sessions* cannot write `Patient`, `Encounter`, `Practitioner`, or `DocumentReference` — the EHR owns that data. Machine-to-machine imports with your API key are not affected. Context resources can also be created at session creation via `fhirContext` items with role `https://tiro.health/fhir/role/new` — see [FHIR Context](./api/session-management#fhir-context).
</Note>
...
More info about context launches can be found on the [session management](./api/session-management) page and the [identity provider](./identity-provider) page.
