...
<Note>
In **EHR-managed data tenants**, direct writes to `Patient`, `Encounter`, `Practitioner`, and `DocumentReference` via the FHIR API are blocked (`403 Forbidden`). Instead, these resources are created automatically when you create a session with `fhirContext` items using the role `https://tiro.health/fhir/role/new` — see [FHIR Context](./api/session-management#fhir-context).
</Note>
