...
The API's follow the [**Structured Data Capturing IG**](https://www.hl7.org/fhir/uv/sdc/index.html) standard for structured data capture, which is a set of FHIR profiles and extensions that standardizes how electronic forms are created, distributed, completed, and processed in healthcare systems.
...
sequenceDiagram
	autonumber
	box Tiro.health Backend Service
	participant text as Text Narrative API
	participant pop as Population API
	participant val as Validation API
	participant tx as Terminology API
	end
	...
	create participant ff as Form Filler
	client ->> ff: Mount the Form Filler with Q & QR
	User ->> ff: Start interacting with form
	loop Background activity during reporting
	ff <<-->>	val: Validated current response
	ff <<-->> text: Generate text narrative
	ff <<-->> tx: Fetch terms and codes
